...
class FollowPetSitterFragment : Fragment() {
    ...
    fun settingRecyclerViewFollowPetSitter(){
        fragmentFollowPetSitterBinding.apply {
...
//                addItemDecoration(deco)
    ...
    inner class FollowPetSitterRecyclerViewAdapter : RecyclerView.Adapter<FollowPetSitterRecyclerViewAdapter.FollowPetSitterViewHolder>(){
        ...
        override fun onBindViewHolder(holder: FollowPetSitterViewHolder, position: Int) {
            ...
            holder.rowFollowPetSitterBinding.imageViewRowFollowPetSitterLike.apply {
                // 토글 버튼 상태 설정
                holder.isChecked = isCheckedList[position]
                updateLikeIcon(this, holder.isChecked)
                setOnClickListener {
                    holder.isChecked = !holder.isChecked
                    isCheckedList[position] = holder.isChecked
                    updateLikeIcon(this, holder.isChecked)
                }
            }
        }

        private var isCheckedList = BooleanArray(50) { false }

        private fun updateLikeIcon(imageView: ImageView, isChecked: Boolean) {
            imageView.setImageResource(if (isChecked) R.drawable.ic_favorite_empty_24px else R.drawable.ic_favorite_fill_24px)
        }
    }
}