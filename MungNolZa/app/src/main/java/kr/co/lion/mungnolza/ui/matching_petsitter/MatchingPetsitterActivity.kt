...
class MatchingPetsitterActivity : AppCompatActivity() {
    ...
    lateinit var activityMatchingPetsitterBinding: ActivityMatchingPetsitterBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        activityMatchingPetsitterBinding = ActivityMatchingPetsitterBinding.inflate(layoutInflater)
        setContentView(activityMatchingPetsitterBinding.root)
    ...
    fun replaceFragment(name: MatchingPetsitterFragmentName, addToBackStack:Boolean, isAnimate:Boolean, data:Bundle?){
        ...
        when(name){
            // 매칭화면
            MatchingPetsitterFragmentName.MATCHING_FRAGMENT -> {
                newFragment = MatchingFragment()
            }
            // 펫시터 프로필 화면
            MatchingPetsitterFragmentName.PETSITTER_INFO_FRAGMENT -> {
                newFragment = PetSitterInfoFragment()
            }
            // 펫시터 리뷰 목록 화면
            MatchingPetsitterFragmentName.PETSITTER_REVIEW_FRAGMENT -> {
                newFragment = PetSitterReviewFragment()
            }
            // 결제 화면
            MatchingPetsitterFragmentName.PAYMENT_FRAGMENT -> {
                newFragment = PaymentFragment()
            }
            // 예약 확정 화면
            MatchingPetsitterFragmentName.RESERVATION_CONFIRMED_FRAGMENT -> {
        ...
        if(newFragment != null){
            ...
            fragmentTransaction.replace(R.id.matchingPetsitterContainer, newFragment!!)
    ...
    fun removeFragment(name: MatchingPetsitterFragmentName){
        SystemClock.sleep(200)
