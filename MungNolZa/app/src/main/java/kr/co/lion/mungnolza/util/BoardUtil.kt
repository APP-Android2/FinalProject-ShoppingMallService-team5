...
class BoardUtil {
    companion object{
        ...
        fun rotateBitmap(bitmap: Bitmap, degree:Float): Bitmap {
            ...
            // 첫 번째 : 원본 이미지
            // 두 번째와 세번 째 : 원본 이미지에서 사용할 부분의 좌측 상단 x,y 좌표
            // 네번째와 다섯번째 : 원본 이미지에서 사용할 부분의 가로 세로 길이
            // 여섯번째 : 변환행렬. 적용해준 변환 행렬이 무엇이냐에 따라 이미지 변형 방식이 달라진다.
        ...
        fun saveImageViewData(context: Context, imageView: ImageView, fileName:String){
            ...
            // 이미지뷰에서 BitmapDrawable 객체를 추출한다.
...
enum class BoardFragmentName(var str:String){
